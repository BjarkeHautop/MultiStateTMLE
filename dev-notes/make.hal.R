...
make.hal <- function(hal.jj) {
  ...
  if (sum(names(hal.jj) %in% c("components", "interactions")) > 0) {
    ...
    if (sum(names(hal.jj) %in% c("interactions")) > 0) {
      ...
      if (
        length(baseline.with <- hal.jj[["interactions"]][["baseline.with"]]) > 0
      ) {
...
compile_hal_spec_to_hal_sl <- function(
  spec,
  tmp.long,
  varnames,
  state.names,
...
hal_spec(
  baseline = c(
    complexity = , # if not specified, will just pick "medium"
    interactions = "none" | "with_baseline" | "with_treatment"
  ), # if not specified, will just pick "none"
  history = list(
    c("counts", "times"),
    complexity = ,
  ...
  time = c(
    complexity = , # if not specified, will just pick
...
hal_spec(
  baseline = "medium",
  history = "medium",
  time = list(complexity = c(), interactions = "history")
)
...
generate.interactions <- function(
  data,
  include = NULL,
  exclude = NULL,
  degree = 5
