...
if (FALSE) {
  ...
  initial.fit <- prepare.initial(
    ...
    fit.types = list(
      Z = list(
        model = "Surv(tstart, tstop, delta == 2)~L0+L+W",
        fit = "cox",
      ...
      outcome1 = list(
        model = "Surv(tstart, tstop, delta == 1)~L0+Z+L",
        fit = "cox"
      ),
      cr1 = list(model = "Surv(tstart, tstop, delta == 5)~L0+Z+L", fit = "cox"),
      censoring = list(
        model = "Surv(tstart, tstop, delta == 0)~L0+Z+L",
        fit = "cox"
      )
    ),
    browse = FALSE,
...
for (misspecify in list(c(), which.misspecify)) {
  ...
  for (m in 1:M) {
    ...
    initial.fit <- prepare.initial(
      ...
      tau = 3,
      fit.types = list(
        Z = list(model = model.Z, fit = "cox", at.risk = function(dt) {
          (dt[["Z"]] == 0)
        }),
        L = list(model = model.L, fit = "cox", at.risk = function(dt) {
          (dt[["L"]] == 0)
        }),
        W = list(model = model.W, fit = "cox", at.risk = function(dt) {
          (dt[["W"]] == 0)
        }),
        outcome1 = list(model = model.1, fit = "cox"),
        cr1 = list(model = model.2, fit = "cox"),
        censoring = list(
          model = "Surv(tstart, tstop, delta == 0)~L0+Z+L",
          fit = "cox"
        )
    ...
    for (alpha in which.alphas) {
      if ("target" %in% target.parameters) {
        ...
        saveRDS(
          est.lists[[as.character(alpha)]],
          file = paste0(
            "./output/",
            "sim-study-1-",
            "estimation-target-alpha",
            alpha,
      ...
      if ("auxiliary" %in% target.parameters) {
        ...
        saveRDS(
          est.aux.lists[[as.character(alpha)]],
          file = paste0(
            "./output/",
            "sim-study-1-",
            "estimation-auxiliary-alpha",
            alpha,
            ifelse("1" %in% misspecify, "-misspecify-1", ""),
            ifelse("Z" %in% misspecify, "-misspecify-Z", ""),
            ifelse("L" %in% misspecify, "-misspecify-L", ""),
...
for (alpha in which.alphas) {
  int_effect <- lapply(1:100, function(rep) {
    ...
    prop_alpha_Z <- mean(data_alpha[,
      any(Delta == 2 & Time < tau)[1],
      by = "ID"
    ][[2]]) # with intervention

    tmp.out <- list(effect_z = prop_alpha_Z, effect_1 = prop_alpha)

    if (any(class(tmp.out) == "try-error")) {
      print(paste0("alpha = ", alpha, ", rep = ", rep))
      return(list(effect_z = NA, effect_1 = NA))
    }
