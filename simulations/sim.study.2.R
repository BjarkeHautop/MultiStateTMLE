...
if (FALSE) {
  ...
  initial.fit <- prepare.initial(
    ...
    fit.types = list(
      Z = list(
        model = "Surv(tstart, tstop, delta == 2)~L0+A0+L",
        fit = "cox",
        at.risk = function(dt) (dt[["Z"]] == 0)
      ),
      L = list(
        model = "Surv(tstart, tstop, delta == 3)~L0+A0+Z",
        fit = "cox",
        at.risk = function(dt) (dt[["L"]] == 0)
      ),
      outcome1 = list(
        model = "Surv(tstart, tstop, delta == 1)~L0+A0+Z+L",
        fit = "cox"
      ),
      censoring = list(
        model = "Surv(tstart, tstop, delta == 0)~L0+A0+Z+L",
        fit = "cox"
      )
    ),
    fit.treatment = "A0~L0",
