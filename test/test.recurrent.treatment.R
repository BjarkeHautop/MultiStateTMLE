...
if (system("echo $USER", intern = TRUE) %in% c("jhl781")) {
  setwd(
    "//projects/biostat01/people/jhl781/research/TMLE-from-2020june/targeted-max-likelihood/"
  )
...
if (system("echo $USER", intern = TRUE) %in% c("jhl781")) {
  setwd(
    "//projects/biostat01/people/jhl781/research/TMLE-from-2020june/targeted-max-likelihood/MultiStateTMLE/"
  )
  .libPaths(c(.libPaths(), "//projects/biostat01/people/jhl781/tmp"))
} else {
...
initial.fit <- prepare.initial(
  ...
  tau = 3,
  fit.types = list(
    Z = list(model = "Surv(tstart, tstop, delta == 2)~L0+L+W+Z", fit = "cox"),
    L = list(
      model = "Surv(tstart, tstop, delta == 3)~L0+Z+W",
      fit = "cox",
      at.risk = function(dt) (dt[["L"]] == 0)
    ),
    W = list(
      model = "Surv(tstart, tstop, delta == 4)~L0+Z+L",
      fit = "cox",
      at.risk = function(dt) (dt[["W"]] == 0)
    ),
    outcome1 = list(
      model = "Surv(tstart, tstop, delta == 1)~L0+Z+L+W",
      fit = "cox"
    ),
    cr1 = list(model = "Surv(tstart, tstop, delta == 5)~L0+Z+L+W", fit = "cox"),
    censoring = list(
      model = "Surv(tstart, tstop, delta == 0)~L0+Z+L+W",
      fit = "cox"
    )
  ),
  max.count = 1,
...
for (alpha in which.alphas) {
  int_effect <- lapply(1:10, function(rep) {
    data_alpha <- simRecurrentTreatment(
      N = N2,
      cens = 0,
    ...
    prop_alpha <- mean(data_alpha[, sum(Delta == 1 & Time < tau), by = "ID"][[
      2
    ]]) # with intervention
    prop_alpha_death <- mean(data_alpha[,
      any(Delta == 5 & Time < tau),
      by = "ID"
    ][[2]]) # with intervention
    prop_alpha_Z <- mean(data_alpha[, sum(Delta == 2 & Time < tau), by = "ID"][[
      2
    ]]) # with intervention

    tmp.out <- list(
      effect_z = prop_alpha_Z,
      effect_1 = prop_alpha,
    ...
    if (any(class(tmp.out) == "try-error")) {
      ...
      return(list(effect_z = NA, effect_1 = NA, effect_2 = NA))
    }
  ...
  print(
    truth.out <- c(
      truth_1 = mean(
        sapply(int_effect, function(rep) rep$effect_1),
        na.rm = TRUE
      ),
      truth_2 = mean(
        sapply(int_effect, function(rep) rep$effect_2),
        na.rm = TRUE
      ),
      truth_z = mean(
        sapply(int_effect, function(rep) rep$effect_z),
        na.rm = TRUE
      )
    )
  )
