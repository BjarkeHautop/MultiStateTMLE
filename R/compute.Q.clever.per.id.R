...
compute.Q.clever.per.id <- function(
  ...
  if (length(pcols) == 0L) {
    warning(
      "No hazard columns found with prefix '",
      P.prefix,
      "'. Returning Q=0"
    )
  ...
  if (length(found_names) == 0L) {
    stop(
      "No hazard columns matched the expected pattern '",
      P.prefix,
      "<name>.<s>'"
    )
  }
  ...
  columns_by_name <- setNames(
    lapply(found_names, function(x) rep(NA_character_, S)),
    found_names
  )
  ...
  for (nm in found_names) {
    ...
    for (s in seq_len(S)) {
      ...
      if (!is.na(colname) && colname %in% names(dt_id)) {
        mat[, s] <- as.numeric(dt_id[[colname]])
      }
    }
    hazard_mats[[nm]] <- mat
  }
  ...
  if (!is.null(process.types)) {
    declared_terminal <- names(process.types)[sapply(
      process.types,
      function(x) identical(x, "terminal")
    )]
    if (length(declared_terminal) > 0L) {
  ...
  if (length(stateful_in_states) > 0L) {
    ...
    if (length(derived_processes) > 0) {
      ...
      key_vec_real <- do.call(
        paste,
        c(lapply(states[, ..stateful_derived], as.character), sep = ",")
      )
    } else {
      key_vec_real <- do.call(
        paste,
        c(lapply(states[, ..stateful_in_states], as.character), sep = ",")
      )
    }
  ...
  if (TRUE) {
    for (j_all in seq_len(M_proc_all)) {
      ...
      if (
        procj_name %in%
          stateful_in_states &
          length(procj_name_s <- grep(procj_name, sstates, value = TRUE))
      ) {
    ...
    for (j_all in seq_len(M_proc_all)) {
      ...
      if (procj_name %in% stateful_in_states) {
        ...
        for (s in seq_len(S)) {
          ...
          if (is_derived) {
            # & parts[j_in] == proc_caps[j_in] ## <- fix: adapt to multiple!
            for (derived_var in grep(
              procj_name,
              derived_processes,
              value = TRUE
            )) {
              j_derived <- which(
                c(state_processes, derived_processes) == derived_var
              )
  ...
  if (identical(parameter, "target")) {
    ...
    if (length(terminal_names) > 0L) {
      target_name <- terminal_names[1L]
    } else {
      target_name <- found_names[1L]
    }
  }
  if (!(target_name %in% c(found_names, state_processes))) {
    stop(
      "parameter must be one of discovered names or a recurrent target declared. Found: ",
      paste(c(found_names, state_processes), collapse = ", ")
    )
  }
  ...
  if (!is.null(process.types) && target_name %in% names(process.types)) {
    ...
    target_type <- if (target_name %in% terminal_names) {
      "terminal"
    } else if (target_name %in% stateful_in_states) {
      "recurrent"
    } else {
      "one.jump"
    }
  ...
  if (length(terminal_names) > 0L) {
    Pout_sum_mat <- Reduce(
      `+`,
      lapply(terminal_names, function(nm) hazard_mats[[nm]]),
  ...
  if (length(stateful_names) > 0L) {
    Pproc_sum_mat <- Reduce(
      `+`,
      lapply(stateful_names, function(nm) hazard_mats[[nm]]),
      init = matrix(0, nrow = Tn, ncol = S)
    )
  } else {
  ...
  for (j in seq_len(M)) {
    gamma_jump_idx_mat[, j] <- gamma_jump_idx[[j]]
  }
  ...
  if (length(time_var_match) > 0) {
    ...
    if (TRUE) {
      ## THIS, is where I am currenty at (April, 2)

      state_times <- states[, time_var_match, with = FALSE]
      inf_rows <- lapply(time_var_match, function(time_var_match_jj) {
        is.infinite(state_times[[(time_var_match_jj)]])
      })
      ...
      for (time_var_match_jj in time_var_match) {
        ...
        rows_by_time[[time_var_match_jj]] <- setNames(
          vector("list", length(uniq_time)),
  ...
  if (FALSE) {
    ...
    get_rows_gamma <- function(tt) {
      key <- paste(
        vapply(time_var_match, function(v) dt_id[[v]][tt], numeric(1)),
        collapse = "|"
      )
      ans <- cache[[key]]
      if (!is.null(ans)) {
  ...
  compute.Q.up.to.Tn <- function(Tn) {
    ...
    if (compute.clever) {
      ...
      if (!target_recurrent && target_name_in_states) {
        ## <- 20/3: this needs checking: !target_recurrent &&
          ...
          if (length(time_var_match) > 0) {
            ...
            for (time_var_match_jj in time_var_match) {
              rows_gamma[[time_var_match_jj]] <-
                rows_by_time[[time_var_match_jj]][[as.character(tt_time[[
                  time_var_match_jj
                ]][tt])]]
            }
            rows_gamma <- apply(do.call("cbind", rows_gamma), 1, function(row) {
              all(row)
            })
            Qn_jump_mat <- matrix(
              Qn[gamma_jump_idx_mat[rows_gamma, ]],
              nrow = S,
              ncol = M
            )
          } else {
    ...
    clever.fun <- function(s_vec) {
      ...
      if (length(terminal_names) > 0L) {
        ...
        if (m > 0) {
          clever1[1:m, terminal_names] <- matrix(
            rep(term_base[1:m], length(terminal_names)),
            nrow = m,
      ...
      if (length(state_processes) > 0L) {
        ...
        if (FALSE) {
          if (length(time_var_match) > 0L) {
            if (TRUE) {
              ...
              s_inf <- st[
                data.table::data.table(state = s_vec, time = Inf),
                row
              ]
              ...
              for (time_var_match_jj in time_var_match) {
                ...
                s_lookup[[time_var_match_jj]] <- data.table::fcoalesce(
                  s_exact,
                  s_inf
                )
              }
              ...
              for (time_var_match_jj in time_var_match) {
                s_jj <- s_vec %in%
                  st[st[[time_var_match_jj]] < Inf][[state.idx.col]]
        ...
        if (m > 0) {
          ...
          # reshape to m x M
          clever1_stateful <- matrix(
            vals_long,
            nrow = m,
            ncol = ncol(succ_idx_mat)
          )
          # add recurrence offsets if present
          if (any(is_recurrent_state_processes != 0L)) {
            clever1_stateful <- clever1_stateful +
              matrix(
                rep(is_recurrent_state_processes, each = m),
                nrow = m,
                ncol = length(is_recurrent_state_processes),
                byrow = FALSE
              )
          }
        ...
        # last row: tt = Tn
        clever1[Tn, state_processes] <- Q_mat[
          Tn,
          as.integer(succ_idx_mat[Tn, ])
        ] +
          (is_recurrent_state_processes * 1) # if recurrence offset (careful with types)
      }

      return(list(clever1 = clever1, clever0 = clever0))
    }
  ...
  if (compute.clever) {
    ...
    for (nm in all_names) {
      ...
      out[[paste0("clever.Q.", nm)]] <- Q.out$clever1[, nm] -
        Q.out$clever0[, nm]
      if (clever.by.state) {
        for (s in seq_len(S)) {
          out[[paste0("clever.Q.", nm, ".", s)]] <-
            Q.out[[paste0("clever1.", s)]][, nm] -
            Q.out[[paste0("clever0.", s)]][, nm]
        }
      }
    }
    for (s in seq_len(S)) {
  ...
  if (get.years.lost) {
    ...
    Q.years.lost <- 0
    clever.years.lost.matrix <- matrix(
      0,
      nrow = Tn,
      ncol = length(all_names),
      dimnames = list(NULL, all_names)
    ...
    if (clever.by.state) {
      ...
      for (s in seq_len(S)) {
        clever.years.lost.list[[s]] <- matrix(
          0,
          nrow = Tn,
          ncol = length(all_names),
          dimnames = list(NULL, all_names)
    ...
    for (tt in Tn:1) {
      ...
      if (clever.by.state) {
        for (s in seq_len(S)) {
          clever_diff <- Q.out.block[[paste0("clever1.", s)]] -
            Q.out.block[[paste0("clever0.", s)]] # matrix m_rep x P
          clever.years.lost.list[[s]][1:imax, ] <- clever.years.lost.list[[s]][
            1:imax,
          ] +
            dt.vec[tt] * out[["at.risk"]][1:imax] * clever_diff[1:imax, ]
        }
    ...
    for (nm in all_names) {
      out[[paste0("clever.Q.years.lost.", nm)]] <- clever.years.lost.matrix[,
        nm
      ]
      if (clever.by.state) {
        for (s in seq_len(S)) {
