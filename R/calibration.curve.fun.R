...
#' @export
calibration.curve.fun <- function(
  initial.fit = NULL,
  a = NULL,
  ...
  alpha.out <- lapply(alpha.grid, function(alpha) {
    if (alpha > 0) {
      calibration.est <- tmle.alpha.fun.fixed(
        parameter = z.name,
        tau = tau.z,
        alpha = alpha,
      ...
      est.deriv.auxiliary <- estimate.derivative(
        ...
        h = 0.3 *
          ...
          sqrt(mean(calibration.est$eic^2))
      )
    ...
    out.target <- lapply(target, function(target1) {
      ...
      return(list(
        ...
        target.se.crude = target.se.crude,
        target.se = target.se,
    ...
    out.estimate <- c(
      alpha = alpha,
      exposure.est = calibration.est$estimate[[grep(
        "tmle.est|one.step.est",
        names(calibration.est$estimate),
        value = TRUE
      )]],
      exposure.se = calibration.est$estimate[["se"]],
      target.est = sapply(out.target, function(out.target.1) {
        as.numeric(out.target.1[["target.est"]][["estimate"]][grep(
          "tmle.est|one.step.est",
          names(out.target.1[["target.est"]][["estimate"]]),
          value = TRUE
        )])
      }), #["tmle.est"]
      target.se = sapply(out.target, function(out.target.1) {
        out.target.1[["target.se"]]
      }) ## ,
    ...
    out.checks <- c(
      ...
      est.deriv.target = sapply(out.target, function(out.target.1) {
        out.target.1[["est.deriv.target"]]
      })
    )

    if (output.eic) {
      target.eic <- lapply(out.target, function(out.target.1) {
        out.target.1[["target.est"]][["eic"]]
      })
      eic <- lapply(out.target, function(out.target.1) {
        out.target.1[["target.est"]][["eic"]] +
          out.target.1[["est.deriv.target"]] * alpha.eic
      })
      if (length(target) == 1) {
