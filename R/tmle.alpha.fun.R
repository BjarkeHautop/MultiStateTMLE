...
tmle.alpha.fun <- function(
  ...
  tau = 1.2,
  alpha = 1,
  z.name = "z", ## old options (but new one not made yet)
  ...
  z.process.id <- (1:length(process.names))[
    tolower(process.names) == tolower(z.name)
  ]
  ...
  if (length(alpha.list) > 0) {
    ...
    for (alpha.kk in 1:length(alpha.list)) {
      tmp.long[,
        ...
        .SDcols = names(formals(alpha.list[[alpha.kk]]))
      ]
    }
    z.present <- TRUE
  }
  ...
  if (length(alpha.list) == 0) {
    if (length(z.process.id) == 0) {
      ...
      if (!identical(alpha, 1)) {
        warning(
          "No 'z' process found in initial.fit; alpha-scaling will be ignored (treated as 1)."
        )
      }
    } else {
      z.present <- TRUE
  ...
  target.id <- (1:length(process.names))[
    tolower(target) == tolower(process.names)
  ]
  ...
  if (target.name %in% names(at.risks)) {
    ...
    if (process.types[target.name] == "one.jump") {
      # <- fixme
  ...
  if (z.present) {
    if (length(alpha.list) > 0) {
      ...
      for (alpha.kk in 1:length(alpha.list)) {
        tmp.long[,
          (paste0("cum.hazard.", names(alpha.list)[alpha.kk])) := cumsum(
            get(paste0("at.risk.", names(alpha.list)[alpha.kk])) *
              get(paste0("P.", names(alpha.list)[alpha.kk]))
          ),
          by = "id"
        ]
        tmp.long[,
          (paste0("cum.hazard.", names(alpha.list)[alpha.kk], ".1")) := c(
            0,
            get(paste0("cum.hazard.", names(alpha.list)[alpha.kk]))[-.N]
          ),
          by = "id"
        ]
        tmp.long[,
          clever.weight.alpha := clever.weight.alpha *
            get(paste0("alpha.", names(alpha.list)[alpha.kk]))^get(names(
              alpha.list
            )[alpha.kk]) *
            exp(
              -(get(paste0("alpha.", names(alpha.list)[alpha.kk])) - 1) *
      ...
      tmp.long[,
        cum.hazard.z := cumsum(
          get(paste0("at.risk.", process.names[z.process.id])) *
            get(paste0("P.", process.names[z.process.id]))
        ),
        by = "id"
      ]
      tmp.long[, cum.hazard.z.1 := c(0, cum.hazard.z[-.N]), by = "id"]
      tmp.long[,
        clever.weight.alpha := alpha^get(z.name) *
          exp(-(alpha - 1) * cum.hazard.z.1)
      ]
    }
    if (truncate.weights > 0) {
      no.truncated <- tmp.long[
        time <= final.time & clever.weight.alpha > truncate.weights,
        length(unique(id))
      ]
      tmp.long[
        time <= final.time & clever.weight.alpha > truncate.weights,
        clever.weight.alpha := truncate.weights
      ]
    }
  } else {
    tmp.long[, clever.weight.alpha := 1]
  ...
  if (length(output.weights) > 0) {
    ...
    if (truncate.weights) {
      out.weights <- c(out.weights, no.truncated = no.truncated)
    }
    ### censoring*a weights
    tmp.cens.weight <- tmp.long[
      time <= final.time,
      max(clever.weight),
      by = "id"
    ]
  ...
  if (length(alpha.list) > 0) {
    for (alpha.kk in 1:length(alpha.list)) {
      for (varname in grep(
        paste0(P.prefix, names(alpha.list)[alpha.kk]),
        names(tmp.long),
        value = TRUE
      )) {
  ...
  for (iter in 1:max.iter) {
    ...
    tmp.long[["gamma.mapping.warning"]] <- (tmp.long[["id"]] == 1) *
      (iter == 1) *
      (verbose)
    ...
    dt_list <- split(
      tmp.long[,
        !(names(tmp.long) %in%
          c("Q", grep(clever.Q.label, names(tmp.long), value = TRUE))),
        with = FALSE
      ],
      by = "id",
      keep.by = TRUE
    ...
    eic <- tmp.long[at.risk == 1, Q[1] - target.est, by = "id"][[2]]

    clever.ids <- (1:length(process.names))[sapply(
      process.names,
      function(process.name) {
        paste0(clever.Q.label, process.name) %in% names(tmp.long)
      }
    )]
      ...
      if (length(alpha.list) > 0) {
        if (name.jj %in% names(alpha.list)) {
          eic <- eic +
            tmp.long[
              at.risk == 1,
              sum(
                get(paste0("alpha.", name.jj)) *
                  clever.weight *
                  clever.weight.alpha *
                  (get(paste0(clever.Q.label, process.names[process.jj]))) *
                  ((delta == process.deltas[process.jj]) -
                    get(paste0("at.risk.", name.jj)) *
                      get(paste0("P.", name.jj)))
              ),
              by = "id"
            ][[2]]
        } else {
        ...
        eic <- eic +
          tmp.long[
            at.risk == 1,
            sum(
              alpha^(process.jj == z.process.id) *
                clever.weight *
                clever.weight.alpha *
                (get(paste0(clever.Q.label, process.names[process.jj]))) *
                ((delta == process.deltas[process.jj]) -
                  get(paste0("at.risk.", name.jj)) * get(paste0("P.", name.jj)))
            ),
            by = "id"
          ][[2]]
      }
  ...
  if (one.step) {
    out <- list(
      estimate = c(one.step.est = one.step.est, se = target.se, g.est = g.est)
    )
  } else {
    out <- list(
      estimate = c(tmle.est = target.est, se = target.se, g.est = g.est)
    )
  }
